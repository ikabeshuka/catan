...
│   ├── treasuresDragonsAdventurers.ts # הגדרות, מטא-דאטה ויעדי ניצחון להרחבת "אוצרות, דרקונים והרפתקנים"
│   └── scenarios/                # תרחישים (סנאריוז) ייעודיים להרחבות המשחק
│       ├── clothForCatan.ts        # תרחיש 6 - Cloth for Catan (בגדים לקטאן)
│       ├── desertDragons.ts        # תרחיש 4 (אוצרות ודרקונים) - Desert Dragons (דרקוני המדבר)
│       ├── enchantedLand.ts        # תרחיש 6 (אוצרות ודרקונים) - Enchanted Land (הארץ המכושפת)
│       ├── fogIsland.ts            # תרחיש 3 - Fog Island
│       ├── fourIslands.ts          # תרחיש 2 - Four Islands
│       ├── greatCanal.ts           # תרחיש 5 (אוצרות ודרקונים) - Great Canal (התעלה הגדולה)
│       ├── greaterCatan.ts         # תרחיש 3 (אוצרות ודרקונים) - Greater Catan (קטאן הגדולה)
│       ├── headingForNewShores.ts  # תרחיש 1 - Heading for New Shores
│       ├── index.ts                # ייצוא מרוכז של התרחישים
│       ├── intoUnknown.ts          # תרחיש 2 (אוצרות ודרקונים) - Into the Unknown (אל הלא-נודע)
...
│       └── treasureIslands.ts      # תרחיש 1 (אוצרות ודרקונים) - Treasure Islands (איי האוצרות)
...
│   └── scenarioState.types.ts    # טיפוסי ניהול המצב הייחודיים עבור תרחישי "אוצרות, דרקונים והרפתקנים"
...
└── components/                   # רכיבי הממשק הוויזואליים (React Components)
    ...
    │       ├── Dragon3D.tsx      # מודל תלת-ממדי של דרקון (אוצרות, דרקונים והרפתקנים)
...
🐉 6. הרחבת "אוצרות, דרקונים והרפתקנים" (Treasures, Dragons & Adventurers)
הרחבה זו משלבת אלמנטים מ"יורדי הים" ומ"ערים ואבירים", וכוללת 6 תרחישים רשמיים מלאים וממומשים בקוד:

✅ תרחישים ומכניקות ממומשות (100% מוכנות)
[x] תרחיש 1: איי האוצרות (Treasure Islands)
גילוי איים נסתרים, מציאת אוצרות המעניקים משאבים, חלקי פיתוח או קלפי קידמה, והקמת יישובים מעבר לים.

[x] תרחיש 2: אל הלא־נודע (Into the Unknown)
הפלגה לא מבוקרת אל הים הלא־נודע, חשיפת אריחים וקבלת החלטות אם להשתמש באוצרות באופן מיידי או לשמר אותם.

[x] תרחיש 3: קטאן הגדולה (Greater Catan)
מכניקה שבה אריחי משאבים באי הבית מדלדלים והמספרים שלהם מוסרים, דבר שמאלץ את השחקנים להתרחב במהירות אל האיים השכנים.

[x] תרחיש 4: דרקוני המדבר (Desert Dragons)
פלישת דרקונים מהמדבר הגדול (מיוצגים תלת-ממדית על ידי Dragon3D.tsx). הדרקונים כובשים אריחים וחוסמים את ייצור המשאבים. השחקנים נדרשים לשלוח אבירים כדי להביס ולגרש אותם בחזרה למדבר.

[x] תרחיש 5: התעלה הגדולה (Great Canal)
פרויקט שיתופי מורכב לחפירת תעלה במערב האי במטרה להשקות מחדש אריחי מדבר חרבים ולהפוך אותם לשדות חיטה פוריים, באמצעות שימוש באבירים פעילים.

[x] תרחיש 6: הארץ המכושפת (Enchanted Land)
לחימה בדרקונים מבוצרת בארץ המכושפת. השחקנים שולחים אבירים חזקים ואדירים כדי להביס דרקונים, לאסוף אוצרות ולזכות בנקודות ניצחון מיוחדות.

🤖 7. שילוב רשת (Multiplayer) ובינה מלאכותית (Gemini AI Bot)
✅ משימות שהושלמו בהצלחה (100% מוכנות)
[x] תשתית חדרים מקוונים: OnlineRoomModal.tsx ליצירת חדרים והצטרפות.

[x] חיבור בזמן אמת (WebSockets): socketService.ts לתקשורת מול שרת ה-Render, כולל מנגנון שדרוגים ותיקוני תאימות לפולינג.
...
[x] סנכרון פרופילים וסטטיסטיקות שרת-לקוח: תשתית Express, Firebase Admin ו-socket.io המאפשרת סנכרון מאובטח ושמירת מצב המשתמש (User Profile) מול בסיס הנתונים של Firestore ישירות דרך ה-Socket.

🏆 8. מנוע חוקי המשחק המרכזי ומערכת הדירוג וההתקדמות
✅ משימות שהושלמו בהצלחה (100% מוכנות)
...
[x] הרחבת "אוצרות, דרקונים והרפתקנים" מיושמת ומובנית ב-100% עם כל 6 התרחישים, כולל מנגנוני דרקונים, חפירת תעלות, איסוף אוצרות ורכיבי תלת-מימד תואמים (Dragon3D.tsx).
...
הצעדים הבאים
להרחיב את כיסוי בדיקות ה-E2E עבור זרימות "ערים ואבירים" ו"אוצרות, דרקונים והרפתקנים" (התקפת ברברים, פלישת דרקונים, חפירת התעלה ושימוש באבירים).

לבצע אופטימיזציית ביצועים נוספת ברינדור מודלי ה-3D המורכבים (אבירים, דרקונים, מטרופולין, ספינת ברברים) במכשירים חלשים.
