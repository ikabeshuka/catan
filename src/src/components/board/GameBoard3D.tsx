...
const Board3DScene: React.FC<Board3DSceneProps> = ({
  ...
  React.useMemo(() => {
    ...
    tileTypes.forEach((type) => {
      ...
      if (tex) {
        // Use ClampToEdgeWrapping and adjust repeat/offset to fit the texture exactly as a hexagonal crop
        tex.wrapS = THREE.ClampToEdgeWrapping;
        tex.wrapT = THREE.ClampToEdgeWrapping;
        tex.repeat.set(1 / 5.9, 1 / 5.9);
  ...
  return (
    <group rotation={[0, 0, 0]}>
      {/* Wooden/Bronze outer frame ring */}
      <mesh position={[0, 0, -0.08]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[18.2, 18.8, 0.15, 64]} />
        <meshStandardMaterial color="#421a0c" roughness={0.8} />
      </mesh>

      {/* Large peripheral sea ring background */}
      <mesh position={[0, 0, -0.06]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[18.0, 18.0, 0.1, 64]} />
        <meshStandardMaterial map={textures.SEA} roughness={0.7} />
      </mesh>
      ...
      {tiles.map((tile) => {
        ...
        return (
          <group key={tile.id}>
            ...
            >
              <extrudeGeometry args={[hexShape, { depth: 0.2, bevelEnabled: false }]} />
              {/* חומר אינדקס 0 - מיועד למשטח העליון והתחתון, עליו נלביש את הטקסטורה החדה */}
              <meshStandardMaterial attach="material-0" map={textures[tile.type as keyof typeof textures]} side={THREE.DoubleSide} roughness={0.5} />
              
              {/* חומר אינדקס 1 - מיועד לקירות והדפנות הצידיים, נצבע אותו בצבע כהה חלק מונע מריחות */}
              <meshStandardMaterial attach="material-1" color="#1e293b" roughness={0.8} />
            </mesh>

            {/* Number Token */}
            {tile.numberToken !== null && tile.numberToken !== undefined && (
              <group position={[tileX, tileY, 0.22]}>
                <mesh 
                ...
                >
                  ...
                  <meshStandardMaterial 
                    color={(tile.numberToken === 6 || tile.numberToken === 8) ? '#fee2e2' : '#fdfaf2'} 
                    roughness={0.6} 
                  />
                </mesh>
                <Text
                  ...
                  fontSize={0.58}
                  color={(tile.numberToken === 6 || tile.numberToken === 8) ? '#b91c1c' : '#1e293b'}
                  fontWeight="bold"
                  anchorX="center"
                  anchorY="middle"
                  onClick={(e) => {
                ...
                <Text
                  ...
                  fontSize={0.23}
                  color={(tile.numberToken === 6 || tile.numberToken === 8) ? '#b91c1c' : '#64748b'}
                  fontWeight="bold"
                  anchorX="center"
                  anchorY="middle"
                >
      ...
      {vertices.map((vertex) => {
        ...
        return (
          <group key={vertex.id} position={[vx, vy, 0.22]}>
            {/* Harbor (Port) Visual Elements */}
            {vertex.isHarbor && (
              <group>
                {/* A cylindrical/circular deck platform for the harbor */}
                <mesh position={[0, 0, -0.05]}>
                  <cylinderGeometry args={[0.32, 0.32, 0.1, 16]} />
                  <meshStandardMaterial color="#5c4033" roughness={0.8} />
                </mesh>
                {/* Text for harbor type */}
                <Text
                  position={[0, 0, 0.08]}
                  fontSize={0.16}
                  color="#ffffff"
            ...
            {/* Hitbox/Visual element for unoccupied vertices (point 3: transparent spheres) */}
            ...
            {/* Click target mesh for occupied/harbor vertices */}
            {(vertex.structure !== 'NONE' || vertex.isHarbor) && (
              <mesh
                position={[0, 0, 0.5 * safeScale3D]} // Slightly lift the sphere to be visible on the surface
                onClick={(e) => {
                  e.stopPropagation();
