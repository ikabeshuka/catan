...
import { OreIcon, SheepIcon, WheatIcon, CardIcon } from '../common/Icons';

export const BuildMenu: React.FC = () => {
  const { turnSubPhase, isCurrentPlayerBot, tradeWithBank, buyDevelopmentCard, undoTurnActions } = useTurnManager();
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        <button
          onClick={() => {
            console.log('[BuildMenu] Buy Dev Card Button Clicked!');
            buyDevelopmentCard();
          }}
        ...
        <div className="flex items-center justify-center gap-3 text-[10px] text-slate-400 bg-slate-950/50 py-1.5 px-3 rounded-lg border border-slate-850">
          ...
          <div className="flex items-center gap-1.5">
            <OreIcon size={13} className="text-slate-400" />
            <span>ברזל x1</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <button
          onClick={() => {
            console.log('[BuildMenu] Undo Actions Button Clicked!');
            undoTurnActions();
          }}
          disabled={isActionsDisabled}
          className={`w-full py-3 px-4 rounded-xl font-extrabold text-xs tracking-wider transition-all duration-300 border border-transparent flex items-center justify-center gap-2
            ${isActionsDisabled 
              ? 'bg-slate-900/60 text-slate-500 cursor-not-allowed opacity-50' 
              : 'bg-rose-600/90 hover:bg-rose-600 text-white hover:brightness-110 active:scale-[0.97] shadow-lg shadow-rose-600/10 hover:shadow-rose-600/25 cursor-pointer'
            }`}
        >
          <span>🔄 בטל פעולות</span>
        </button>
      </div>
